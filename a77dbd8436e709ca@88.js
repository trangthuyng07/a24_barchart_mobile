...
{
  const container = html`<div style="padding: 20px; font-family: 'Inter', sans-serif;"></div>`;
  ...
  const topSection = html`<div style="
    display: flex;
  ...
  // ===== Bottom Section: Grid Gallery =====
  const grid = html`<div style="
    display: grid;
 ...
 overflow-y: auto;
  ...
  select.onchange = () => {
    ...
    svg.on("click", () => {
      ...
      svg.selectAll("text.bar-label").data(shuffled, d => d.director)
        .transition().duration(800).attr("y", d => y(d.director) + y.bandwidth() / 2);
